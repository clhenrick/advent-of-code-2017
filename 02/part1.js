...
function parseTable(t) {
  // convert the template literal to a 2D array of numeric values
  return t.split('\n')
    .map(row => row.split('\t')
...
function checksum(data) {
  // sum the difference of the min and max values for each row in a table
  return data.reduce((acc, cur) => {
    const min = minArray(cur);
    const max = maxArray(cur);
    const diff = max - min;
    acc += diff;
    return acc;
  }, 0);
